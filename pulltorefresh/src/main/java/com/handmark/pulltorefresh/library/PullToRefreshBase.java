...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	static final int AUTOLOAD_LAST_ITEM_POSITION = 10;
	...
	protected boolean inLoad = false;

	/**
	 * 自动刷新模式,可选下拉到底部或上拉到顶部。
	 */
	protected Mode mAutoLoadMode = Mode.PULL_FROM_END;
	// ===========================================================
	// Constructors
	// ===========================================================
	...
	protected void onRefreshing(final boolean doScroll) {
		if(Mode.PULL_FROM_END == mAutoLoadMode && inLoad){
			if (mMode.showFooterLoadingLayout()) {
				mCurrentMode = Mode.PULL_FROM_END;
				mFooterLayout.refreshing();
			}
		}
		else if(Mode.PULL_FROM_START == mAutoLoadMode && inLoad){
