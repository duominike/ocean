...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView> extends PullToRefreshBase<T> implements OnScrollListener {
    ...
    private OnLastItemVisibleListener mOnLastItemVisibleListener = new OnLastItemVisibleListener() {
        ...
        public void onLastItemVisible() {
            Log.d(LOG_TAG, "onLastItemVisible: mAudoLoadMode--->>" + mAutoLoadMode);
            Log.d(LOG_TAG, "onLastItemVisible: ShowFooterLoadingLayout--->>" + getMode().showFooterLoadingLayout());
            Log.d(LOG_TAG, "onLastItemVisible: ShowHeaderLoadingLayout--->>" + getMode().showHeaderLoadingLayout());
            if (mAutoLoadMode == Mode.PULL_FROM_END && !getMode().showFooterLoadingLayout())
                return;
            if (mAutoLoadMode == Mode.PULL_FROM_START && !getMode().showHeaderLoadingLayout())
                return;
            if (inLoad || mLastVisableItemPos <= 0)
                return;
            inLoad = true;
            int count = getRefreshableView() != null ? getRefreshableView().getCount() : 0;
            if (count > 0 && count != m_iCount) {
                m_iCount = count;
                setRefreshing(false);
                if (DEBUG) {
                    Log.d(LOG_TAG, "onLastItemVisible");
//				Toast.makeText(getContext(), "正在加载", Toast.LENGTH_SHORT).show();
    ...
    private int mLastVisableItemPos = AUTOLOAD_LAST_ITEM_POSITION;        //default auto load position
    private boolean mOnScrollToLoad = true;
    ...
    public void setAutoLoadLastVisableItemPos(int pos) {
        if (pos <= 0) {
            mLastVisableItemPos = 0;
            return;
        }
        mLastVisableItemPos = pos;
    }

    public void setOnScrollToLoad(boolean is) {
        mOnScrollToLoad = is;
    }

    public void setAutoLoadMode(Mode mode) {
        if (mode == Mode.PULL_FROM_START) {
            mAutoLoadMode = mode;
        } else {
            mAutoLoadMode = Mode.PULL_FROM_END;
        }
    }
    ...
    public final void onScroll(final AbsListView view, final int firstVisibleItem, final int visibleItemCount, final int totalItemCount) {
         ...
         */
        if (null != mOnLastItemVisibleListener && mLastVisableItemPos > 0) {
            if (mAutoLoadMode == Mode.PULL_FROM_START)
                mLastItemVisible = (totalItemCount > 0) && (firstVisibleItem <= mLastVisableItemPos);
            else if (mAutoLoadMode == Mode.PULL_FROM_END)
                mLastItemVisible = (totalItemCount > 0) && (firstVisibleItem + visibleItemCount >= totalItemCount - mLastVisableItemPos);
        }

        /**
         * 滚动时调用加载回调。
         */
        if (mOnScrollToLoad && firstVisibleItem != 0 && null != mOnLastItemVisibleListener && mLastItemVisible) {
    ...
    public final void onScrollStateChanged(final AbsListView view, final int state) {
         ...
         * visible.
         */
        if (!mOnScrollToLoad && state == OnScrollListener.SCROLL_STATE_IDLE && null != mOnLastItemVisibleListener && mLastItemVisible) {
            mOnLastItemVisibleListener.onLastItemVisible();
        }
